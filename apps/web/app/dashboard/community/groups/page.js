...
export default function GroupsPage() {
  ...
  const DUMMY_GROUPS = [
    ...
    // Add canChat permission to groups - some groups are restricted to founders only, others allow all members
    const DUMMY_GROUPS_WITH_PERMS = DUMMY_GROUPS.map((g, idx) => ({
      ...g,
      canChat: idx % 2 === 0 // Even indices can chat, odd indices are read-only
    }));
