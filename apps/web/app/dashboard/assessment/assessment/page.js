...
const DUMMY_ASSIGNMENTS = [
  {
    _id: 'a1',
    title: 'Financial Model v1.0',
    description: 'Build a comprehensive 3-year financial projection including revenue streams, COGS, and burn rate analysis. Ensure all assumptions are documented.',
  ...
  },
  {
    ...
    description: 'Outline your product development milestones for the next 12 months alongside a detailed go-to-market strategy for the first 3 cohorts.',
    submissionStatus: 'Pending',
  ...
  },
  {
    ...
    description: 'Synthesize findings from at least 15 problem-discovery interviews. Map the persona and pain points in detail.',
    submissionStatus: 'Pending',
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        let fetchedTasks = taskJson.success ? taskJson.data : [];
        if (fetchedTasks.length === 0) fetchedTasks = DUMMY_ASSIGNMENTS;
  ...
  return (
    <div className="platform-page" style={{ padding: '0.5rem 2.5rem' }}>
      <header className="platform-page-header" style={{ marginBottom: '1.5rem' }}>
        <h1 className="platform-page-title" style={{ fontSize: '2rem', marginBottom: '4px' }}>Project Assessments</h1>
        <p className="platform-page-subtitle" style={{ fontSize: '0.9rem', color: '#64748B' }}>Submit your startup's core artifacts for executive evaluation.</p>
      </header>
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', borderTop: '1px solid #F1F5F9', paddingTop: '1.5rem', marginTop: 'auto', marginBottom: '1.5rem' }}>
        ...
        <div>
          <span style={{ fontSize: '0.65rem', color: '#94A3B8', fontWeight: 900, display: 'block', lineHeight: 1 }}>DEADLINE</span>
          <span style={{ fontSize: '0.9rem', color: '#111', fontWeight: 900 }}>{task.deadline ? new Date(task.deadline).toLocaleDateString() : 'Rolling'}</span>
        </div>
      </div>

      <button onClick={() => onSelect(task)} className="btn-brand-primary" style={{ width: '100%', padding: '14px', borderRadius: '14px', fontSize: '0.9rem', fontWeight: 900 }}>
        Launch Assessment
      </button>
    </motion.div>
  );
