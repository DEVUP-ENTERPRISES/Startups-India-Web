...
function MentorDashboardContent() {
  ...
  const setActiveTab = (tab) => {
    ...
    router.push(`/mentor/dashboard?tab=${tab}`);
  };
  ...
  useEffect(() => {
    async function load() {
      ...
      if (userRes.data?.user) {
        ...
        try {
          const [profileRes, requestsRes, sessionsRes, messagesRes, resourcesRes, reviewsRes, earningsRes, analyticsRes, menteesRes] = await Promise.all([
            apiGet('/api/v1/mentors/profile'),
            apiGet('/api/v1/mentors/dashboard-requests'),
            apiGet('/api/v1/mentors/sessions'),
            apiGet('/api/v1/mentors/messages'),
            apiGet('/api/v1/mentors/resources'),
            apiGet('/api/v1/mentors/reviews'),
            apiGet('/api/v1/mentors/earnings'),
            apiGet('/api/v1/mentors/analytics'),
            apiGet('/api/v1/mentors/mentees')
          ]);

          if (profileRes.data && profileRes.data.fullName) setProfileData(profileRes.data);
          if (requestsRes.data && requestsRes.data.length > 0) setRequests(requestsRes.data);
          if (sessionsRes.data && sessionsRes.data.length > 0) setSessions(sessionsRes.data);
          if (messagesRes.data && messagesRes.data.length > 0) {
            setMessages(messagesRes.data);
            setActiveChatId(messagesRes.data[0]._id || messagesRes.data[0].id);
          }
          if (resourcesRes.data && resourcesRes.data.length > 0) setResources(resourcesRes.data);
          if (reviewsRes.data && reviewsRes.data.length > 0) setReviews(reviewsRes.data);
          if (earningsRes.data && (earningsRes.data.balance > 0 || earningsRes.data.payouts?.length > 0)) {
            setEarnings(earningsRes.data);
            setEarningsBalance(earningsRes.data.balance || 0);
          }
          if (analyticsRes.data && analyticsRes.data.totalSessions > 0) setAnalytics(analyticsRes.data);
          if (menteesRes.data && menteesRes.data.length > 0) setMentees(menteesRes.data);
        } catch (err) {
  ...
  const handleAcceptRequest = async (id) => {
    const patchRes = await apiPatch(`/api/v1/mentors/dashboard-requests/${id}`, { status: 'accepted' });
    if (!patchRes.error) {
      const [requestsRes, sessionsRes, analyticsRes] = await Promise.all([
        apiGet('/api/v1/mentors/dashboard-requests'),
        apiGet('/api/v1/mentors/sessions'),
        apiGet('/api/v1/mentors/analytics')
      ]);
      ...
      addToast('Approved mentorship request!');
    } else {
  ...
  const handleDeclineRequest = async (id) => {
    const patchRes = await apiPatch(`/api/v1/mentors/dashboard-requests/${id}`, { status: 'declined' });
    if (!patchRes.error) {
      setRequests(prev => prev.filter(r => (r._id || r.id) !== id));
      addToast('Mentorship request declined.', 'warning');
    } else {
      addToast('Failed to decline request', 'warning');
    }
  };
  ...
  const handleSaveProfile = async (e) => {
    e.preventDefault();
    const patchRes = await apiPatch('/api/v1/mentors/profile', {
  ...
  const handleSendTicket = async (e) => {
    ...
    const res = await apiPost('/api/v1/mentors/support', supportTicket);
    if (!res.error) {
  ...
  return (
    <div style={s.layoutContainer}>
      ...
      <aside style={s.sidebar}>
        ...
        <nav style={s.sidebarMenu}>
          {[
            ...
            { id: 'requests', label: 'Requests', icon: <UserCheck size={18} />, badge: requests.length },
            { id: 'sessions', label: 'Sessions', icon: <Video size={18} /> },
            { id: 'calendar', label: 'Calendar', icon: <Calendar size={18} /> },
            { id: 'mentees', label: 'My Mentees', icon: <Users size={18} /> },
            { id: 'messages', label: 'Messages', icon: <MessageSquare size={18} />, badge: unreadMsgCount, badgeColor: '#C5975B' },
            ...
            <button 
              ...
              style={s.sidebarBtn(activeTab === item.id)}
            >
              ...
              {item.badge > 0 && (
                <span style={s.badgeLabel(item.badgeColor || '#ef4444')}>{item.badge}</span>
              )}
            </button>
          ))}
        </nav>
      ...
      <div style={s.mainWorkspace}>
        ...
        <header style={s.workspaceHeader}>
          <div>
            <h1 style={{ ...s.welcomeText, display: 'flex', alignItems: 'center', gap: '8px' }}>
              {activeTab === 'dashboard' ? (
                <>
          ...
          <div style={s.headerRight}>
            <div style={s.searchBar}>
              ...
              <input type="text" placeholder="Search anything...    Ctrl + K" style={s.searchInput} />
            </div>

            <button onClick={() => setActiveTab('messages')} style={s.iconButton}>
              <MessageSquare size={18} color="#64748b" />
              {unreadMsgCount > 0 && <span style={s.headerBadge} />}
            </button>

            <button onClick={() => setActiveTab('notifications')} style={s.iconButton}>
              <Bell size={18} color="#64748b" />
              {unreadNotifCount > 0 && <span style={s.headerBadge} />}
            </button>

            <div style={s.headerUser} onClick={() => setActiveTab('profile')}>
              <div style={s.avatar(40)}>{profileData.fullName.split(' ').map(n=>n[0]).join('')}</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
              ...
              <div style={s.dashboardStatsRow}>
                <div style={s.statCard}>
                  ...
                  <div>
                    ...
                    <span style={s.statTrendGreen}>↑ 3 this month</span>
                  </div>
                </div>
                ...
                <div style={s.statCard}>
                  ...
                  <div>
                    ...
                    <span style={s.statTrendGreen}>↑ 2 this week</span>
                  </div>
                </div>
              ...
              <div style={s.gridSplit}>
                ...
                <div style={s.splitLeft}>
                  ...
                  {/* Widget: Earnings Overview */}
                  <div style={s.widgetCard}>
                    <div style={s.widgetHeader}>
                      <h3 style={s.widgetTitle}>Earnings Overview</h3>
                      <button onClick={() => setActiveTab('earnings')} style={s.widgetHeaderLink}>This Month</button>
                    </div>
                    
                    <div style={s.earningsMainRow}>
                      <div>
                        <div style={s.earningsLabel}>Total Earnings</div>
                        <div style={s.earningsValue}>₹48,750</div>
                        <span style={s.statTrendGreen}>↑ 18% vs last month</span>
                      </div>
                      <div style={s.earningsIconBox}>
                        <Wallet size={24} color="#C5975B" />
                      </div>
                    </div>

                    <div style={s.earningsGrid}>
                      <div style={s.earningsSubBox}>
                        <div style={s.earningsSubTitle}>Paid Sessions</div>
                        <div style={s.earningsSubValue}>12</div>
                      </div>
                      <div style={s.earningsSubBox}>
                        <div style={s.earningsSubTitle}>Pending Payments</div>
                        <div style={s.earningsSubValue}>₹8,000</div>
                      </div>
                      <div style={s.earningsSubBox}>
                        <div style={s.earningsSubTitle}>Total Payouts</div>
                        <div style={s.earningsSubValue}>₹40,750</div>
                      </div>
                    </div>
                    <button onClick={() => setActiveTab('earnings')} style={s.widgetFooterBtn}>View Earnings →</button>
                  </div>
