...
export default function DashboardSidebar({
  ...
  useEffect(() => {
    // Derive which section should be auto-opened based on pathname alone.
    // Do NOT include `navigation` in the dependency array — it is a new array
    // object on every render, which would trigger this effect on every render
    // and prevent `openSectionId` from ever sticking after a click.
    const allSections = user?.role === 'mentor' ? mentorNavigation
      : user?.role === 'investor' ? investorNavigation
        : studentNavigation;
